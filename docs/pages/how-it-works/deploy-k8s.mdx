...
DevPod works the same with kubernetes as with Machines, the key difference is the "tunnel" is set up using the kubernetes control plane (e.g. kubectl ...) so an agent is not necessary
...
<figure>
  ...
  <figcaption>DevPod - Component Diagram VMs</figcaption>
</figure>