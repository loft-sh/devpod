...
Prebuilding a workspace means building a ready-to-use docker image out of the `devcontainer.json`, its referenced features and a potentially linked `Dockerfile`.
...
DevPod will use the current provider for doing this, means you can also use remote providers to prebuild an image. You can even have a separate provider for just prebuilding images.

## Using Prebuilds

Using prebuilds means you specify a docker image repository, where DevPod will search for an image with a specific hash generated out of the devcontainer configuration. You can either specify this prebuild repository via a flag during workspace creation or directly in the `devcontainer.json`.
