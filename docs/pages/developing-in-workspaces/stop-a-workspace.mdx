...
Stopping a workspace means to temporarily pause a container to not consume any more computing resources such as computing or memory.
Depending on the used provider, this either means to stop a container (see [docker stop](https://docs.docker.com/engine/reference/commandline/stop/) for more information)
or shutdown the underlying VM where the container is running in.
...
For example, the google cloud provider will shutdown the virtual machine where the workspace is running on after its unused for 10 minutes by default.
