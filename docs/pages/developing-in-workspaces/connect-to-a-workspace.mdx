...
After a workspace is successfully created, the workspace is reachable via the ssh host `${devcontainerId}.devpod`.
...
:::info SSH Fallback
If for whatever reason this does not work you can also use the regular SSH connection with `${devcontainerId}.devpod` to connect VSCode with a workspace
:::
...
:::info SSH Fallback
If for whatever reason this does not work you can also use the regular SSH connection with `${devcontainerId}.devpod` to connect your JetBrains IDE with a workspace
:::

:::info Fleet Support
Fleet currently only works by manually adding an SSH connection with `${devcontainerId}.devpod`
:::

### SSH

Upon workspace creation, DevPod will automatically modify the `~/.ssh/config` to include an entry for `${devcontainerId}.devpod`, which allows you to use the following command to connect to your workspace:
```
ssh ${devcontainerId}.devpod
```

This also allows you to connect any IDE that supports remote development through SSH via the given host `${devcontainerId}.devpod`.
