...
After a workspace is successfully created, the workspace is reachable via the ssh host `my-workspace.devpod`.
...
To select a different openvscode version, run via:
```
...
This section shows additional commands to configure DevPod's behaviour when opening a workspace.
...
You can list all supported IDEs by DevPod via:
```
devpod ide list
```
