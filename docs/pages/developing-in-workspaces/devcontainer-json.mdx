...
When working on the `devcontainer.json` itself, it's important to understand when DevPod will apply the new configuration.
...
* Running `devpod up my-workspace`: This will apply all new configuration that is not related on how to start or build the container and will apply immediately on the next start. This includes: port-forwarding, startup commands and remote environment variables. The Desktop app equivalent for this is the 'Open' button.
