...
When a user executes a non-trivial CLI command, one that loads the configuration, the CLI will send an event about the command and basic information about the execution environment (Operating system, CPU architecture, DevPod version, ect.). Another event is sent if a command fails, or when it takes longer than 10 seconds to complete. The events are sent to our self-hosted backend.  
...
```yaml
{
  "type":"cmdfinished",                   # type of event, other type is "cmdstarted"
  "event":{
...
To disable the telemetry execute following command:
