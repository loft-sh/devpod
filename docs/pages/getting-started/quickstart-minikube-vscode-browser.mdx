---
title: Quickstart Minikube VS Code
sidebar_label: Quickstart Minikube VS Code
---
...
cd ~/devpods
touch devpod-pv.yml
vim devpod-pv.yml # or your prefered text editor
```
...
Create a directory for the persisent volume
...
**Note**
This is a simplification for a production setup where a kubeconfig should be provided for a kubernetes context, namepace and users. You will also need kubectl installed where your Devpods client is running from. The kubernetes provider uses the underlying kubectl providing the locally stored kubeconfig to access a remote cluster. This setup requires kubernetes administration support.

### Identify your namepaces
