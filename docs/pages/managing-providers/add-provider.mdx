...
Some providers are available for auto-installation:
...
- A File URL to `provider.yaml`
- A File Path to `provider.yaml`
...
To manage options from CLI, you can list the table of provider's options by using:
...
```sh
devpod provider set-options <provider-name> --single-machine
```
