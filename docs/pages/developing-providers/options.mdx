...
- `command`: A command to retrieve the option value automatically. Can also reference other variables in the command, e.g. `echo ${MY_OTHER_VAR}-suffix`. For compatability reasons this command will be executed in an emulated shell on windows.
...
command launched on your machine. Can also reference other variables in the command, e.g. `echo ${MY_OTHER_VAR}-suffix`. For compatability reasons this command will be executed in an emulated shell on windows.
...
Predefined options:
- **DEVPOD**: Absolute path to the current DevPod CLI binary.
