...
import {
    ...
    DEVPOD_COMMAND_HELPER, DEVPOD_COMMAND_INIT,
    DEVPOD_COMMAND_LIST, DEVPOD_COMMAND_OPTIONS,
    DEVPOD_COMMAND_PROVIDER, DEVPOD_COMMAND_SET_OPTIONS, DEVPOD_COMMAND_USE,
    DEVPOD_FLAG_DEBUG, DEVPOD_FLAG_JSON_LOG_OUTPUT,
    DEVPOD_FLAG_JSON_OUTPUT, DEVPOD_FLAG_NAME, DEVPOD_FLAG_OPTION, DEVPOD_FLAG_USE
} from "./constants";
...
export class ProviderCommands {
    ...
    static async ListProviders(): Promise<Result<TProviders>> {
        const result = await new Command([DEVPOD_COMMAND_PROVIDER, DEVPOD_COMMAND_LIST, DEVPOD_FLAG_JSON_OUTPUT]).run()
        if (result.err) {
            return result
        }
    ...
    static async GetProviderID(source: string) {
        const result = await new Command([DEVPOD_COMMAND_HELPER, DEVPOD_COMMAND_GET_PROVIDER_NAME, source]).run()
        if (result.err) {
            return result
        }
    ...
    static async AddProvider(rawProviderSource: string, config: TAddProviderConfig): Promise<ResultError> {
        ...
        if (!isOk(result.val)) {
            ...
            return Return.Failed(
                maybeOutput?.message || `Failed to add provider with source ${rawProviderSource}: ${result.val.stderr}`,
            )
        }
    ...
    static async RemoveProvider(id: TProviderID) {
        const result = await ProviderCommands.newCommand([
            DEVPOD_COMMAND_PROVIDER, DEVPOD_COMMAND_OPTIONS, id, DEVPOD_FLAG_JSON_OUTPUT
        ]).run()
        if (result.err) {
            return result
        }
    ...
    static async InitProvider(id: TProviderID) {
        const result = await ProviderCommands.newCommand([
            DEVPOD_COMMAND_PROVIDER, DEVPOD_COMMAND_INIT, id, DEVPOD_FLAG_JSON_LOG_OUTPUT
        ]).run()
        if (result.err) {
            return result
        }
