...
CGO_ENABLED=1 GOOS=linux GOARCH=arm64 go build -race -ldflags "-s -w" -o $BUILD_DIR/devpod-linux-arm64 $SRC_DIR
CGO_ENABLED=1 GOOS=darwin GOARCH=arm64 go build -race -ldflags "-s -w" -o $BUILD_DIR/devpod-darwin-arm64 $SRC_DIR
CGO_ENABLED=1 GOOS=darwin GOARCH=amd64 go build -race -ldflags "-s -w" -o $BUILD_DIR/devpod-darwin-amd64 $SRC_DIR
