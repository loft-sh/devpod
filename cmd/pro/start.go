...
func (cmd *StartCmd) waitForDeployment(ctx context.Context) (*corev1.Pod, error) {
	...
	loftPod, err := waitForPodReady(ctx, cmd.KubeClient, cmd.Namespace, cmd.Log)
...
func waitForPodReady(ctx context.Context, kubeClient kubernetes.Interface, namespace string, log log.Logger) (*corev1.Pod, error) {
	...
	now := time.Now()
	pod := &corev1.Pod{}
	err := wait.PollUntilContextTimeout(ctx, time.Second*2, loft.Timeout(), true, func(ctx context.Context) (bool, error) {
