...
func (cmd *SSHCmd) startTunnel(ctx context.Context, devPodConfig *config.Config, containerClient *ssh.Client, workspaceClient client2.BaseWorkspaceClient, log log.Logger) error {
	...
	if cmd.Proxy || cmd.Stdio {
		if cmd.Proxy {
			...
			go func() {
				if err := cmd.setupLoftPlatformAccess(ctx, containerClient, workspaceClient, cmd.Context, cmd.Provider, log); err != nil {
					log.Error(err)
				}
			}()
		}
...
func (cmd *SSHCmd) setupLoftPlatformAccess(ctx context.Context, sshClient *ssh.Client, workspaceClient client2.BaseWorkspaceClient, context, provider string, log log.Logger) error {
